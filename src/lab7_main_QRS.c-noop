...
#define BUFFER_SIZE 1024  // Define a buffer size for storing samples

// sBuffers for storing samples and QRS states
static int samples[BUFFER_SIZE] = {0};       // Circular buffer for samples
static int dual_qrs_values[BUFFER_SIZE] = {0}; // Circular buffer for dual QRS
static int buffer_index = 0;                 // Index for circular buffer
...
void task_main_loop (void *pvParameters) {
    ...
    int THRESHOLD = 2500;  // Replace with your desired threshold
    int current_start_idx = -1, current_peak_idx = -1, current_end_idx = -1;
    ...
    for ( ;; ) {
		...
		if (sample > THRESHOLD) {
			if (!detecting_peak) {
				...
				detecting_peak = true;
				current_start_idx = buffer_index; // Start with current index

				// Look backward for the Q point
				int idx = buffer_index;
				for (int i = 1; i < BUFFER_SIZE; ++i) {
					if (samples[idx] < samples[current_start_idx]) {
				...
				current_peak_idx = buffer_index; // R point (initially the current index)
			}
			else {
				// Update the peak during detection
				if (samples[buffer_index] > samples[current_peak_idx]) {
					current_peak_idx = buffer_index; // Update the R point
				}
			}
		} 
		else {
			if (detecting_peak) {
				// End of the QRS detection
				detecting_peak = false;
				current_end_idx = buffer_index; // Start with current index

				// Look forward for the S point
				int idx = buffer_index;
				for (int i = 1; i < BUFFER_SIZE; ++i) {
					idx = (idx + 1) % BUFFER_SIZE;
					if (samples[idx] < samples[current_end_idx]) {
						current_end_idx = idx;
					} else {
						break;
					}
				...
				// Set dual_QRS for the QRS region (from Q to S)
				int idx = current_start_idx;
				while (idx != (current_end_idx + 1) % BUFFER_SIZE) {
					dual_qrs_values[idx] = 1;  // Set dual_QRS high for this region
					idx = (idx + 1) % BUFFER_SIZE;
				}

				// Output QRS detection to GPIO
				digitalWrite(D6, 1); // Turn on GPIO during QRS
			}
		}

		// Reset GPIO outside of QRS
		if (!detecting_peak) {
			digitalWrite(D6, 0); // Turn off GPIO
		}
